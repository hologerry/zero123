...
from my3d import depth_smooth_loss, get_T
...
from voxnerf.vis import stitch_vis, vis_img
...
class SJC(BaseConf):
    family: str = "sd"
    sd: SD = SD(variant="objaverse", scale=100.0)
    lr: float = 0.05
    n_steps: int = 10000
...
def sjc_3d(
    ...
    **kwargs,
    ...
    folder_name = (
        prefix
        + "/scene-%s-index-%d_scale-%s_train-view-%s_view-weight-%s_depth-smooth-wt-%s_near-view-wt-%s"
    ...
    background_mask, image_mask = input_mask == 0.0, input_mask != 0.0
    ...
    background_mask = cv2.resize(
        background_mask.astype(np.float32), dsize=(H, W), interpolation=cv2.INTER_NEAREST
    ).astype(bool)
    ...
    with tqdm(total=n_steps) as pbar, HeartBeat(pbar) as hbeat, EventStorage(folder_name) as metric:

        with torch.no_grad():

            tforms = transforms.Compose([transforms.Resize(256), transforms.CenterCrop((256, 256))])
        ...
        for i in range(n_steps):
            ...
            emptiness_loss = emptiness_loss * (1.0 + emptiness_multiplier * i / n_steps)
        ...
        metric.put_artifact("ckpt", ".pt", lambda fn: torch.save(vox.state_dict(), fn))
...
def evaluate(score_model, vox, poser):
    ...
    metric.put_artifact("view_seq", ".mp4", lambda fn: stitch_vis(fn, read_stats(metric.output_dir, "view")[1]))
