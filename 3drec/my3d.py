...
def depth_smooth_loss(depth):
    ...
    return (grad_x.abs().mean() + grad_y.abs().mean()) / 2.0
...
def camera_pose(eye, front, up):
    ...
    pose = np.block([[x, y, z, eye], [0, 0, 0, 1]])
    return pose
...
def compute_proj_to_normalized(aspect, fov):
    ...
    partial_ndc_intrinsic = np.array(
        [[1 / (t * aspect), 0, 0, 0], [0, 1 / t, 0, 0], [0, 0, -1, 0]]  # copy the negative distance for division
    )
...
def compute_normalized_to_img_trans(aspect, img_height_in_pix):
    ...
    img_y_coord_flip = np.array([[1, 0, 0], [0, -1, img_h - 1], [0, 0, 1]])  # note the -1
