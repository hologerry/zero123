...
def make_nfp_data(base_path):
    ...
    datasets = [
        NfpDataset(x, image_transforms=copy.copy(tforms), default_caption="A view from a train window") for x in dirs
    ]
...
class ObjaverseDataModuleFromConfig(pl.LightningDataModule):
    def __init__(
        self, root_dir, batch_size, total_view, train=None, validation=None, test=None, num_workers=4, **kwargs
    ):
        super().__init__(self)
    ...
    def val_dataloader(self):
        dataset = ObjaverseData(
            root_dir=self.root_dir, total_view=self.total_view, validation=True, image_transforms=self.image_transforms
        )
        sampler = DistributedSampler(dataset)
...
class ScalarFlowStaticDataModuleFromConfig(pl.LightningDataModule):
    ...
    def test_dataloader(self):
        return wds.WebLoader(
            ScalarFlowStaticData(root_dir=self.root_dir, total_view=self.total_view, validation=self.validation),
            batch_size=self.batch_size,
            num_workers=self.num_workers,
            shuffle=False,
        )
...
class ScalarFlowStaticData(Dataset):
    def __init__(
        self,
        root_dir="./data/ScalarFlow/zero123_dataset",
        image_transforms=[],
        ext="png",
        default_trans=torch.zeros(3),
        postprocess=None,
