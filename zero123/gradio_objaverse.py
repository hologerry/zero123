...
def main(
    ...
    if preprocess:
        ...
        input_im[input_im[:, :, -1] <= 0.9] = [1.0, 1.0, 1.0, 1.0]  # very important, thresholding background
        input_im = input_im[:, :, :3]
    ...
    x_samples_ddim = sample_model(
        input_im, model, sampler, precision, h, w, ddim_steps, n_samples, scale, ddim_eta, x, y, z
    )
    output_ims = []
